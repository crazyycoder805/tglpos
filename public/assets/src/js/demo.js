...
$(function () {
    ...
    var $themePrimary = $("<li />", { style: "padding: 5px;" }).append(
        '<a href="javascript:void(0)" data-theme="theme-primary" style="background: #0052cc; display: block;vertical-align: middle;" class="clearfix rounded w-p100 h-40 mb-5" title="Theme primary">' +
            "</a>"
    );
    $skinsList.append($themePrimary);

    var $themeInfo = $("<li />", { style: "padding: 5px;" }).append(
        '<a href="javascript:void(0)" data-theme="theme-info" style="background: #00baff; display: block;vertical-align: middle;" class="clearfix rounded w-p100 h-40 mb-5" title="Theme info">' +
            "</a>"
    );
    $skinsList.append($themeInfo);

    var $themeSuccess = $("<li />", { style: "padding: 5px;" }).append(
        '<a href="javascript:void(0)" data-theme="theme-success" style="background: #51ce8a; display: block;vertical-align: middle;" class="clearfix rounded w-p100 h-40 mb-5" title="Theme success">' +
            "</a>"
    );
    $skinsList.append($themeSuccess);

    var $themeDanger = $("<li />", { style: "padding: 5px;" }).append(
        '<a href="javascript:void(0)" data-theme="theme-danger" style="background: #f2426d; display: block;vertical-align: middle;" class="clearfix rounded w-p100 h-40 mb-5" title="Theme danger">' +
            "</a>"
    );
    $skinsList.append($themeDanger);

    var $themeWarning = $("<li />", { style: "padding: 5px;" }).append(
        '<a href="javascript:void(0)" data-theme="theme-warning" style="background: #fec801; display: block;vertical-align: middle;" class="clearfix rounded w-p100 h-40 mb-5" title="Theme warning">' +
            "</a>"
    );
...
(function () {
    ...
    pinIcon.forEach((item, index) => {
        ...
        var InitialLocalStorage = JSON.parse(
            localStorage.getItem("pins") || false
        );
        ...
        item.addEventListener("click", (event) => {
            var localStoragePins = JSON.parse(
                localStorage.getItem("pins") || false
            );
            ...
            if (localStoragePins?.length) {
                if (item.parentNode.classList.contains("pined")) {
                    ...
                    localStoragePins?.includes(linkName) &&
                        localStoragePins.splice(
                            localStoragePins.indexOf(linkName),
                            1
                        );
                }
